...
const LandingPageCatalogue = () => {
  ...
  // View & Filter States
  const [currentView, setCurrentView] = useState(VIEW_GALLERY);
  ...
  // Sync browser fullscreen change event
  useEffect(() => {
    const handleFullscreenChange = () => {
      if (!document.fullscreenElement && isFullScreenLivePreview) {
        // Handled via top bar buttons
      }
    };
  ...
  // Launch Fullscreen Live Preview
  const handleLaunchFullScreenPreview = () => {
    ...
    try {
      ...
      console.warn('Native fullscreen not available:', e);
    }
  };
  ...
  const renderEditorContent = () => (
    <div
      className={`w-full flex-1 min-h-0 flex flex-col gap-2.5 overflow-hidden ${
        isEditorExpanded
          ? 'w-screen h-screen p-4 bg-slate-100 dark:bg-[#0c0d10] fixed inset-0 z-[2147483647]'
          : ''
      }`}
    >
      ...
      <div className="w-full flex-1 min-h-0 rounded-2xl bg-black border border-black flex flex-col overflow-hidden shadow-sm">
        {/* Full Ribbon Toolbar */}
        <div className="w-full px-3 py-1.5 flex flex-wrap items-center justify-between gap-1 text-white flex-shrink-0 select-none bg-black border-b border-white/10">
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[15px] font-voda-exb tracking-normal text-white dark:text-black uppercase">
              LANDING PAGE CATALOGUE
            </h1>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-[13px] font-voda-exb text-white/80 dark:text-black/80">
        ...
        {currentView === VIEW_GALLERY && (
          <>
            {/* ── 2. INFO NOTICE STRIP (Shown only in Gallery View) ── */}
            <div
              className={`w-full py-2 px-4 rounded-xl border flex items-center gap-2.5 flex-shrink-0 text-[11px] font-medium shadow-2xs ${
                isDark ? 'bg-[#202127]/80 text-slate-300 border-white/10' : 'bg-[#f1f3f7] text-slate-700 border-slate-200'
              }`}
            >
              ...
              <span>
                Browse, preview, and manage simulated landing page templates directly on the cards or create responsive awareness pages using the rich canvas designer.
              </span>
            </div>

            {/* ── 3. MAIN WORKSPACE CONTAINER (Gallery Only) ── */}
            <div
              className={`w-full flex-1 rounded-2xl border p-4 flex flex-col gap-3 overflow-hidden shadow-xs ${
                isDark ? 'bg-[#202127]/80 border-white/10' : 'bg-[#ffffff] border-slate-200'
              }`}
            >
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div
                      className={`flex items-center rounded-xl overflow-hidden border shadow-2xs ${
                        isDark ? 'bg-[#202127]/80 text-white border-white/10' : 'bg-[#f1f3f7] text-slate-900 border-slate-300/80'
                      }`}
                    >
                      <input
                        type="text"
                      ...
                      >
                        <option value="Name" className={isDark ? 'bg-[#15161A] text-white' : 'bg-white text-slate-900'}>
                          SEARCH BY NAME
                        </option>
        ...
        {/* ========================================================================= */}
        {/* ── 🌟 SAVE CHOICE DIALOGUE MODAL ("Full Screen Preview" or "Submit")      */}
        {/* ========================================================================= */}
        {showSaveChoiceModal && (
        ...
        {/* ========================================================================= */}
        {/* ── 🌟 COMPLETE LAPTOP SCREEN LIVE PREVIEW VIA REACT PORTAL                */}
        {/* ========================================================================= */}
        {isFullScreenLivePreview &&
          typeof document !== 'undefined' &&
          createPortal(
            ...
            >
              ...
              >
                <button
                  ...
                  title="Exit Full Screen Preview"
                >
                  <X className="w-5 h-5 stroke-[2.5]" />
                </button>
              </div>

              {/* 100% Full-Viewport Interactive Employee Simulation Preview */}
              <iframe
        ...
        {/* ========================================================================= */}
        {toastMessage && (
          <div className="fixed bottom-6 right-6 z-50 px-4 py-2 rounded-xl bg-[#2E7D32] text-white text-xs font-voda font-bold shadow-xl flex items-center gap-2">
